...
ARCHIVE_BASE=$FOGHAT_ARCHIVE_DIR/mpyle-href-vis
ARCHIVE_DIR=$ARCHIVE_BASE/latest
TODAY=`date -u '+%Y%m%d'`
LOG_FILE="$FOGHAT_LOG_DIR/vis_archive-$TODAY.log"

mkdir -p $FOGHAT_LOG_DIR $ARCHIVE_DIR

#/usr/bin/wget -nv --no-parent -r --limit-rate=5m --wait=5 --timestamping --append-output=$LOG_FILE -nd --directory-prefix=$ARCHIVE_DIR/$date $URL
